...
#else
                uint32_t insn = INSN_TO_LE(*(branch_mod[i]));
                int32_t imm24 = insn & 0x00ffffff;

                if (imm24 & 0x00800000)
                    imm24 |= ~0x00ffffff;

                imm24 += distance;
                insn = (insn & 0xff000000) | (imm24 & 0x00ffffff);
                *(branch_mod[i]) = INSN_TO_LE(insn);
#endif
            }
...
static void __attribute__((unused)) put_to_stream(void *d, char c)
{
    char **pptr = (char**)d;
    char *ptr = *pptr;
