...
{
    return RA_IsARMRegisterMapped(arm_reg);
}
...
{
    for (int i = 0; i < 16; i++) {
        snapshot->m68k_arm_reg[i] = LRU_M68kRegisters[i].rs_ARMReg;
        snapshot->m68k_dirty[i] = LRU_M68kRegisters[i].rs_Dirty;
    }

    for (int i = 0; i < 8; i++) {
        snapshot->lru_table[i] = LRU_Table[i];
        snapshot->fpu_reg_state[i] = FPU_Reg_State[i];
    }

    snapshot->register_pool = register_pool;
    snapshot->changed_mask = changed_mask;
    snapshot->fpu_alloc_state = FPU_AllocState;
    snapshot->got_cc = got_CC;
    snapshot->mod_cc = mod_CC;
    snapshot->reg_fpcr = reg_FPCR;
    snapshot->mod_fpcr = mod_FPCR;
    snapshot->reg_fpsr = reg_FPSR;
    snapshot->mod_fpsr = mod_FPSR;
}

void RA_RestoreState(const RAStateSnapshot *snapshot)
{
    for (int i = 0; i < 16; i++) {
        LRU_M68kRegisters[i].rs_ARMReg = snapshot->m68k_arm_reg[i];
        LRU_M68kRegisters[i].rs_Dirty = snapshot->m68k_dirty[i];
    }

    for (int i = 0; i < 8; i++) {
        LRU_Table[i] = snapshot->lru_table[i];
        FPU_Reg_State[i] = snapshot->fpu_reg_state[i];
    }

    register_pool = snapshot->register_pool;
    changed_mask = snapshot->changed_mask;
    FPU_AllocState = snapshot->fpu_alloc_state;
    got_CC = snapshot->got_cc;
    mod_CC = snapshot->mod_cc;
    reg_FPCR = snapshot->reg_fpcr;
    mod_FPCR = snapshot->mod_fpcr;
    reg_FPSR = snapshot->reg_fpsr;
    mod_FPSR = snapshot->mod_fpsr;
}
