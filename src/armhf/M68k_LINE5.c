...
uint32_t *EMIT_DBcc(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr)
{
    ...
    else
    {
        ...
        *ptr++ = add_immed(c_true, REG_PC, off);
        ...
        /* If condition is true, DBcc falls through without decrementing the counter. */
        if (m68k_condition != M_CC_F)
        {
            arm_condition = EMIT_TestCondition(&ptr, m68k_condition);

            /* On ARM32 we need an explicit conditional move for the fallthrough PC. */
#ifdef __aarch64__
            *ptr++ = csel(REG_PC, c_true, c_false, arm_condition);
#else
            *ptr++ = mov_cc_reg(arm_condition, REG_PC, c_true);
#endif
        ...
        /* DBcc decrements only the low 16 bits and branches while the result != -1. */
        uint8_t reg = RA_AllocARMRegister(&ptr);
        *ptr++ = uxth(reg, counter_reg, 0);
        *ptr++ = subs_immed(reg, reg, 1);
        *ptr++ = bfi(counter_reg, reg, 0, 16);
        RA_SetDirtyM68kRegister(&ptr, opcode & 7);

        /* Underflowed counter means loop finished; otherwise continue at the branch target. */
#ifdef __aarch64__
...
#else
        *ptr++ = mov_cc_reg(ARM_CC_MI, REG_PC, c_true);
        *ptr++ = mov_cc_reg(ARM_CC_PL, REG_PC, c_false);
        branch_2 = ptr;
        *ptr++ = b_cc(ARM_CC_PL, 0);
        if (branch_1) {
            *branch_1 = b_cc(arm_condition, ptr - branch_1 - 2);
        }
#endif
...
#else
        *branch_2 = b_cc(ARM_CC_PL, ptr - branch_2 - 2);
#endif
