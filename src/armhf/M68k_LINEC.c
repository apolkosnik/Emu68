...
#ifndef __aarch64__
static uint32_t *EMIT_ABCD_arm32(uint32_t *ptr, uint8_t src, uint8_t dst, uint8_t cc,
                                 uint8_t update_mask, uint8_t result, uint8_t low,
                                 uint8_t tmp_a, uint8_t tmp_b);
#endif
...
#else
    ptr = EMIT_ABCD_arm32(ptr, src, dst, cc, update_mask, tmp_a, tmp_c, tmp_b, tmp_d);
    *ptr++ = bfi(dst, tmp_a, 0, 8);
#endif
...
static uint32_t *EMIT_ABCD_arm32(uint32_t *ptr, uint8_t src, uint8_t dst, uint8_t cc,
                                 ...
                                 uint8_t tmp_a, uint8_t tmp_b)
{
    *ptr++ = and_immed(result, src, 0xff);
    *ptr++ = and_immed(low, src, 0x0f);
    *ptr++ = and_immed(tmp_a, dst, 0xff);
    *ptr++ = and_immed(tmp_b, dst, 0x0f);

    *ptr++ = add_reg(result, result, tmp_a, 0);
    *ptr++ = add_reg(low, low, tmp_b, 0);

    *ptr++ = tst_immed(cc, SR_X);
    *ptr++ = add_cc_immed(ARM_CC_NE, result, result, 1);
    *ptr++ = add_cc_immed(ARM_CC_NE, low, low, 1);

    *ptr++ = cmp_immed(low, 9);
    *ptr++ = add_cc_immed(ARM_CC_HI, result, result, 6);

    *ptr++ = cmp_immed(result, 0x99);
    if (update_mask & SR_XC)
    {
        *ptr++ = bic_immed(cc, cc, SR_XC);
        *ptr++ = orr_cc_immed(ARM_CC_HI, cc, cc, SR_XC);
    }
    *ptr++ = add_cc_immed(ARM_CC_HI, result, result, 0x60);

    if (update_mask & SR_Z)
    {
        *ptr++ = ands_immed(tmp_a, result, 0xff);
...
static uint32_t *EMIT_ABCD_mem(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr)
{
    ...
    *ptr++ = strb_offset(an_dst, tmp_b, 0);
    ...
    ptr = EMIT_ABCD_arm32(ptr, tmp_a, tmp_b, cc, update_mask, tmp_c, tmp_d, tmp_a, tmp_b);
    *ptr++ = strb_offset(an_dst, tmp_c, 0);
