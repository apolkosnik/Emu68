...
static uint32_t *EMIT_MOVEfromSR(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = b_cc(A64_CC_EQ, 23);
    ...
    ptr = EMIT_StoreToEffectiveAddress(ptr, 2, &cc, opcode & 0x3f, *m68k_ptr, &ext_words, 0);
    ...
    *ptr++ = INSN_TO_LE(0xfffffffe);
    ...
    /* MOVE from SR is privileged on this path. */
...
static uint32_t *EMIT_MOVEtoSR(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    uint8_t cc = RA_ModifyCC(&ptr);
    uint8_t tmp = RA_AllocARMRegister(&ptr);
    uint8_t format = RA_AllocARMRegister(&ptr);
    ...
    *ptr++ = INSN_TO_LE(0xffffffff);
    ...
    uint32_t *branch_skip_m = NULL;
    uint32_t *branch_load_isp = NULL;
    uint32_t *branch_after_msp = NULL;
    uint32_t *branch_skip_s = NULL;
    uint32_t *branch_user_from_isp = NULL;
    ...
    uint32_t *user_from_isp_ptr = NULL;

    RA_SetDirtyM68kRegister(&ptr, 15);
    ptr = EMIT_FlushPC(ptr);

    /* MOVE to SR is privileged on the ARM32 path too. */
    *ptr++ = ubfx(tmp, cc, SRB_S, 1);
    *ptr++ = tst_immed(tmp, 1);
    ...
    *ptr++ = mov_reg(changed, cc);
    *ptr++ = movw_immed_u16(tmp, 0xf71f);

    ptr = EMIT_LoadFromEffectiveAddress(ptr, 2, &src, opcode & 0x3f, *m68k_ptr, &ext_words, 1, NULL);

    *ptr++ = and_reg(cc, tmp, src, 0);
    *ptr++ = eor_reg(changed, changed, cc, 0);

    /* Handle ISP/MSP bank changes while remaining in supervisor mode. */
    *ptr++ = ubfx(tmp, changed, SRB_M, 1);
    *ptr++ = tst_immed(tmp, 1);
    branch_skip_m = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    ...
    *ptr++ = tst_immed(tmp, 1);
    branch_load_isp = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    *ptr++ = str_offset(ctx, sp, __builtin_offsetof(struct M68KState, ISP));
    *ptr++ = ldr_offset(ctx, sp, __builtin_offsetof(struct M68KState, MSP));
    branch_after_msp = ptr;
    *ptr++ = b_cc(ARM_CC_AL, 0);
    load_isp_ptr = ptr;
    *branch_load_isp = b_cc(ARM_CC_EQ, load_isp_ptr - branch_load_isp - 2);
    *ptr++ = str_offset(ctx, sp, __builtin_offsetof(struct M68KState, MSP));
    *ptr++ = ldr_offset(ctx, sp, __builtin_offsetof(struct M68KState, ISP));
    after_m_ptr = ptr;
    *branch_after_msp = b_cc(ARM_CC_AL, after_m_ptr - branch_after_msp - 2);
    *branch_skip_m = b_cc(ARM_CC_EQ, after_m_ptr - branch_skip_m - 2);

    /* Supervisor can only flip S from 1 to 0 here; preserve the supervisor bank before loading USP. */
    *ptr++ = ubfx(tmp, changed, SRB_S, 1);
    *ptr++ = tst_immed(tmp, 1);
    branch_skip_s = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    ...
    *ptr++ = ubfx(tmp, cc, SRB_M, 1);
    *ptr++ = tst_immed(tmp, 1);
    branch_user_from_isp = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    *ptr++ = str_offset(ctx, sp, __builtin_offsetof(struct M68KState, MSP));
    *ptr++ = ldr_offset(ctx, sp, __builtin_offsetof(struct M68KState, USP));
    branch_after_user_from_msp = ptr;
    *ptr++ = b_cc(ARM_CC_AL, 0);
    user_from_isp_ptr = ptr;
    *branch_user_from_isp = b_cc(ARM_CC_EQ, user_from_isp_ptr - branch_user_from_isp - 2);
    *ptr++ = str_offset(ctx, sp, __builtin_offsetof(struct M68KState, ISP));
    *ptr++ = ldr_offset(ctx, sp, __builtin_offsetof(struct M68KState, USP));
    after_s_ptr = ptr;
    *branch_after_user_from_msp = b_cc(ARM_CC_AL, after_s_ptr - branch_after_user_from_msp - 2);
    *branch_skip_s = b_cc(ARM_CC_EQ, after_s_ptr - branch_skip_s - 2);
    ...
    *ptr++ = (uint32_t)(uintptr_t)branch_end;
    *ptr++ = 1;
    *ptr++ = 0;
    *ptr++ = INSN_TO_LE(0xfffffffe);
...
static uint32_t *EMIT_LINK16(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = mov_reg(reg, sp);
    ...
    if (offset != 0)
    {
        *ptr++ = movw_immed_u16(displ, offset & 0xffff);
        if (offset < 0)
            *ptr++ = movt_immed_u16(displ, 0xffff);
        *ptr++ = add_reg(sp, sp, displ, 0);
    }
#endif
    RA_SetDirtyM68kRegister(&ptr, 15);
...
static uint32_t *EMIT_RESET(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = (uint32_t)(uintptr_t)branch_success;
    *ptr++ = 1;
    *ptr++ = 0;
    *ptr++ = INSN_TO_LE(0xfffffffe);
    *ptr++ = INSN_TO_LE(0xffffffff);
...
static uint32_t *EMIT_STOP(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = mov_reg(changed, cc);
    *ptr++ = movw_immed_u16(tmp, new_sr);
    *ptr++ = eor_reg(changed, changed, tmp, 0);
    *ptr++ = mov_reg(cc, tmp);
    ...
    *branch_skip_m = b_cc(ARM_CC_EQ, after_m_ptr - branch_skip_m - 2);
    ...
    /* Supervisor can only clear S here; preserve the active supervisor bank before loading USP. */
    *ptr++ = ubfx(tmp, changed, SRB_S, 1);
    *ptr++ = tst_immed(tmp, 1);
    branch_skip_s = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    ...
    *ptr++ = add_immed(REG_PC, REG_PC, 4);
    /* End the unit after updating SR/PC so the dispatcher can resume from STOP. */
...
static uint32_t *EMIT_RTE(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = INSN_TO_LE(0xffffffff);
    ...
    uint8_t changed = RA_AllocARMRegister(&ptr);
    uint32_t *branch_privilege = NULL;
    uint32_t *branch_format_eq2 = NULL;
    uint32_t *branch_format_eq0 = NULL;
    uint32_t *branch_format_fail = NULL;
    uint32_t *branch_skip_extra = NULL;
    uint32_t *branch_end_norm = NULL;
    uint32_t *branch_end_format = NULL;
    uint32_t *format_error_ptr = NULL;
    uint32_t *privilege_ptr = NULL;
    ...
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    ...
    /* Only format 0 and format 2 frames are emitted on the ARM32 path today. */
    *ptr++ = ldrh_offset(sp, format, 6);
    *ptr++ = lsr_immed(format, format, 12);
    *ptr++ = cmp_immed(format, 2);
    branch_format_eq2 = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    *ptr++ = cmp_immed(format, 0);
    branch_format_eq0 = ptr;
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    branch_format_fail = ptr;
    *ptr++ = b_cc(ARM_CC_AL, 0);

    {
        uint32_t *format_ok = ptr;

        *branch_format_eq2 = b_cc(ARM_CC_EQ, format_ok - branch_format_eq2 - 2);
        *branch_format_eq0 = b_cc(ARM_CC_EQ, format_ok - branch_format_eq0 - 2);

        ptr = EMIT_ResetOffsetPC(ptr);

        /* Pop SR and rebuild the unaligned 32-bit PC from two halfwords. */
        *ptr++ = ldrh_offset(sp, changed, 0);
        *ptr++ = ldrh_offset(sp, REG_PC, 2);
        *ptr++ = lsl_immed(REG_PC, REG_PC, 16);
        *ptr++ = ldrh_offset(sp, tmp, 4);
        *ptr++ = orr_reg(REG_PC, REG_PC, tmp, 0);
        *ptr++ = add_immed(sp, sp, 8);

        /* Format 2 carries one additional longword payload after the vector word. */
        *ptr++ = cmp_immed(format, 2);
        branch_skip_extra = ptr;
        *ptr++ = b_cc(ARM_CC_NE, 0);
        *ptr++ = add_immed(sp, sp, 4);
        *branch_skip_extra = b_cc(ARM_CC_NE, ptr - branch_skip_extra - 2);

        /* The restored ARM32 path currently uses supervisor-mode frames only. */
        *ptr++ = mov_reg(cc, changed);

        *ptr++ = ubfx(tmp, cc, SRB_S, 1);
        *ptr++ = tst_immed(tmp, 1);
        {
            uint32_t *branch_user = ptr;
            uint32_t *branch_isp = NULL;
            uint32_t *branch_after_bank_msp = NULL;
            uint32_t *branch_after_bank_isp = NULL;
            uint32_t *user_bank_ptr = NULL;
            uint32_t *after_bank_ptr = NULL;
            ...
            *ptr++ = tst_immed(tmp, 1);
            branch_isp = ptr;
            *ptr++ = b_cc(ARM_CC_EQ, 0);
            *ptr++ = ldr_offset(REG_CTX, sp, __builtin_offsetof(struct M68KState, MSP));
            branch_after_bank_msp = ptr;
            *ptr++ = b_cc(ARM_CC_AL, 0);
            *branch_isp = b_cc(ARM_CC_EQ, ptr - branch_isp - 2);
            *ptr++ = ldr_offset(REG_CTX, sp, __builtin_offsetof(struct M68KState, ISP));
            branch_after_bank_isp = ptr;
            *ptr++ = b_cc(ARM_CC_AL, 0);
            user_bank_ptr = ptr;
            *branch_user = b_cc(ARM_CC_EQ, user_bank_ptr - branch_user - 2);
            *ptr++ = ldr_offset(REG_CTX, sp, __builtin_offsetof(struct M68KState, USP));
            after_bank_ptr = ptr;
            *branch_after_bank_msp = b_cc(ARM_CC_AL, after_bank_ptr - branch_after_bank_msp - 2);
            *branch_after_bank_isp = b_cc(ARM_CC_AL, after_bank_ptr - branch_after_bank_isp - 2);
        }

        *ptr++ = str_offset(REG_CTX, sp, __builtin_offsetof(struct M68KState, A[7]));
        *ptr++ = strh_offset(REG_CTX, cc, __builtin_offsetof(struct M68KState, SR));
        *ptr++ = str_offset(REG_CTX, REG_PC, __builtin_offsetof(struct M68KState, PC));

        branch_end_norm = ptr;
        *ptr++ = b_cc(ARM_CC_AL, 0);
    }

    format_error_ptr = ptr;
    ptr = EMIT_Exception(ptr, VECTOR_FORMAT_ERROR, 0);
    *ptr++ = str_offset(REG_CTX, sp, __builtin_offsetof(struct M68KState, A[7]));
    *ptr++ = strh_offset(REG_CTX, cc, __builtin_offsetof(struct M68KState, SR));
    *ptr++ = str_offset(REG_CTX, REG_PC, __builtin_offsetof(struct M68KState, PC));
    branch_end_format = ptr;
    *ptr++ = b_cc(ARM_CC_AL, 0);
    ...
    *branch_privilege = b_cc(ARM_CC_EQ, privilege_ptr - branch_privilege - 2);
    *branch_format_fail = b_cc(ARM_CC_AL, format_error_ptr - branch_format_fail - 2);
    *branch_end_norm = b_cc(ARM_CC_AL, unit_end - branch_end_norm - 2);
    *branch_end_format = b_cc(ARM_CC_AL, unit_end - branch_end_format - 2);

    RA_FreeARMRegister(&ptr, changed);
    RA_FreeARMRegister(&ptr, format);
    RA_FreeARMRegister(&ptr, tmp);
#endif
...
static uint32_t *EMIT_RTS(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    */
    ...
    if (ret_addr != (uint16_t *)0xffffffff)
    {
        ...
        *ptr++ = movw_immed_u16(reg, ret & 0xffff);
        if ((ret >> 16) & 0xffff)
            *ptr++ = movt_immed_u16(reg, ret >> 16);
        ...
        switch (opcode2 & 0xfff)
        {
            ...
            case 0x801: // VBR
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, VBR));
                break;
            case 0x002: // CACR
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, CACR));
                break;
            case 0x803: // MSP
                sp = RA_MapM68kRegister(&ptr, 15);
                RA_SetDirtyM68kRegister(&ptr, 15);
                ...
                *ptr++ = ubfx(tmp, cc, SRB_M, 1);
                *ptr++ = tst_immed(tmp, 1);
                *ptr++ = mov_cc_reg(ARM_CC_NE, sp, reg);
                RA_FreeARMRegister(&ptr, tmp);
                break;
            case 0x804: // ISP
                sp = RA_MapM68kRegister(&ptr, 15);
                ...
                *ptr++ = ubfx(tmp, cc, SRB_M, 1);
                *ptr++ = tst_immed(tmp, 1);
                *ptr++ = mov_cc_reg(ARM_CC_EQ, sp, reg);
                RA_FreeARMRegister(&ptr, tmp);
                break;
            case 0x0ea: /* JITSCFTHRESH - Maximal number of JIT units for "soft" cache flush */
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, JIT_SOFTFLUSH_THRESH));
            ...
            case 0x0eb: /* JITCTRL - JIT control register */
                tmp = RA_AllocARMRegister(&ptr);
                *ptr++ = and_immed(tmp, reg, 1);
                *ptr++ = str_offset(ctx, tmp, __builtin_offsetof(struct M68KState, JIT_CONTROL));
                RA_FreeARMRegister(&ptr, tmp);
                break;
            case 0x003: // TCR
                *ptr++ = strh_offset(ctx, reg, __builtin_offsetof(struct M68KState, TCR));
                break;
            case 0x004: // ITT0
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, ITT0));
                break;
            case 0x005: // ITT1
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, ITT1));
                break;
            case 0x006: // DTT0
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, DTT0));
                break;
            case 0x007: // DTT1
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, DTT1));
                break;
            case 0x805: // MMUSR
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, MMUSR));
                break;
            case 0x806: // URP
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, URP));
                break;
            case 0x807: // SRP
                *ptr++ = str_offset(ctx, reg, __builtin_offsetof(struct M68KState, SRP));
                break;
            default:
                ptr = EMIT_Exception(ptr, VECTOR_ILLEGAL_INSTRUCTION, 0);
                illegal = 1;
    ...
    else
    {
        switch (opcode2 & 0xfff)
        {
            ...
            case 0x803: // MSP
                ...
                *ptr++ = ubfx(tmp, cc, SRB_M, 1);
                *ptr++ = tst_immed(tmp, 1);
                *ptr++ = mov_cc_reg(ARM_CC_NE, reg, sp);
                RA_FreeARMRegister(&ptr, tmp);
                break;
            case 0x804: // ISP
                sp = RA_MapM68kRegister(&ptr, 15);
                ...
                *ptr++ = ubfx(tmp, cc, SRB_M, 1);
                *ptr++ = tst_immed(tmp, 1);
                *ptr++ = mov_cc_reg(ARM_CC_EQ, reg, sp);
                RA_FreeARMRegister(&ptr, tmp);
                break;
            case 0x0e7: /* JITSIZE - size of JIT cache, in bytes */
                *ptr++ = ldr_offset(ctx, reg, __builtin_offsetof(struct M68KState, JIT_CACHE_TOTAL));
...
static uint32_t *EMIT_NBCD(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *tbz_pos = tbnz(cc, SRB_X, ptr - tbz_pos);
    ...
    uint8_t src_plus_x = RA_AllocARMRegister(&ptr);
    uint8_t tmp = RA_AllocARMRegister(&ptr);
    ...
    if ((opcode & 0x38) == 0)
    {
        ...
        *ptr++ = and_immed(src, dn, 0xff);
    }
    else
    {
        ptr = EMIT_LoadFromEffectiveAddress(ptr, 0, &ea, opcode & 0x3f, *m68k_ptr, &ext_words, 0, NULL);

        /* -(An) mode needs the explicit predecrement load when using size 0 EAs. */
        if ((opcode & 0x38) == 0x20)
        {
            *ptr++ = ldrb_offset_preindex(ea, src, (opcode & 7) == 7 ? -2 : -1);
            RA_SetDirtyM68kRegister(&ptr, 8 + (opcode & 7));
        }
        else
        {
            *ptr++ = ldrb_offset(src, ea, 0);
        }
    }

    *ptr++ = mov_reg(src_plus_x, src);
    *ptr++ = rsb_immed(result, src, 0);

    *ptr++ = tst_immed(cc, SR_X);
    *ptr++ = b_cc(ARM_CC_EQ, 1);
    *ptr++ = sub_immed(result, result, 1);
    *ptr++ = add_immed(src_plus_x, src_plus_x, 1);

    *ptr++ = ands_immed(tmp, src_plus_x, 0x0f);
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    *ptr++ = sub_immed(result, result, 6);

    *ptr++ = cmp_immed(src_plus_x, 0);
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    *ptr++ = sub_immed(result, result, 0x60);

    if (update_mask & SR_XC)
    {
        *ptr++ = bic_immed(cc, cc, SR_XC);
        *ptr++ = cmp_immed(src_plus_x, 0);
        ptr = EMIT_SetFlagsConditional(ptr, cc, SR_XC, ARM_CC_NE);
    }

    if (update_mask & SR_Z)
    {
        *ptr++ = ands_immed(tmp, result, 0xff);
        *ptr++ = bic_cc_immed(ARM_CC_NE, cc, cc, SR_Z);
    }
    ...
    else
    {
        if ((opcode & 0x38) == 0x18)
        {
            *ptr++ = strb_offset_postindex(ea, result, (opcode & 7) == 7 ? 2 : 1);
            RA_SetDirtyM68kRegister(&ptr, 8 + (opcode & 7));
        }
        else
        {
    ...
    RA_FreeARMRegister(&ptr, src);
    RA_FreeARMRegister(&ptr, src_plus_x);
    RA_FreeARMRegister(&ptr, tmp);
...
static uint32_t *EMIT_CHK(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr, uint16_t *insn_consumed)
{
    ...
    *ptr++ = INSN_TO_LE(0xfffffffe);
    ...
    /* Check if Dn < 0. */
    *ptr++ = cmp_immed(cmp_dn, 0);

    branch_negative = ptr;
    *ptr++ = b_cc(ARM_CC_MI, 0);

    /* Check if Dn <= src. */
    *ptr++ = cmp_reg(src, cmp_dn);

    branch_skip_exception = ptr;
    *ptr++ = b_cc(ARM_CC_GE, 0);

    /* Upper-bound failure clears N before the CHK exception frame is built. */
    *ptr++ = bic_immed(cc, cc, SR_N);
    branch_after_clear = ptr;
    *ptr++ = b_cc(ARM_CC_AL, 0);

    negative_ptr = ptr;
    *branch_negative = b_cc(ARM_CC_MI, negative_ptr - branch_negative - 2);
    *ptr++ = orr_immed(cc, cc, SR_N);

    *branch_after_clear = b_cc(ARM_CC_AL, ptr - branch_after_clear - 2);
    RA_SaveState(&exception_state);
    ptr = EMIT_Exception(ptr, VECTOR_CHK, 2, opcode_address);
    RA_RestoreState(&exception_state);
    unit_end = ptr;
...
static struct OpcodeDef InsnTable[4096] = {
    ...
    [0x800 ... 0x807] = { { .od_EmitMulti = EMIT_NBCD }, NULL, SR_XZ, SR_XZC, 1, 0, 1 },
    [0x810 ... 0x839] = { { .od_EmitMulti = EMIT_NBCD }, NULL, SR_XZ, SR_XZC, 1, 1, 1 },
