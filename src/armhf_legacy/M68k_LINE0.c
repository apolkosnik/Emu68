        ...
        switch (size)
        {
            ...
            case 4:
                if (immediate)
                    ...
                    *ptr++ = cmp_reg(dest, immed);
                break;
            case 2:
                *ptr++ = lsl_immed(tmpreg, dest, 16);
                *ptr++ = cmp_reg(tmpreg, immed);
                break;
            case 1:
                *ptr++ = lsl_immed(tmpreg, dest, 24);
                *ptr++ = cmp_reg(tmpreg, immed);
                break;
#endif
        }
...
uint32_t *EMIT_CMP2(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr)
{
    ...
    *ptr++ = subs_reg(31, higher, reg,  LSL, 0);
    ...
    uint8_t higher = RA_AllocARMRegister(&ptr);
    uint8_t reg = RA_MapM68kRegister(&ptr, opcode2 >> 12);
    uint8_t cmp_value = reg;
    uint32_t *branch_skip_upper = NULL;
    ...
    switch ((opcode >> 9) & 3)
    {
        ...
        case 1:
            ...
            *ptr++ = ldrsh_offset(ea, higher, 2);
            break;
        case 2:
            *ptr++ = ldr_offset(ea, lower, 0);
            *ptr++ = ldr_offset(ea, higher, 4);
            break;
    }

    /* Data-register form sign-extends the tested operand for byte/word sizes. */
    if ((opcode2 & 0x8000) == 0)
    {
        switch ((opcode >> 9) & 3)
        {
            case 0:
                cmp_value = RA_AllocARMRegister(&ptr);
                *ptr++ = sxtb(cmp_value, reg, 0);
                break;
            case 1:
                cmp_value = RA_AllocARMRegister(&ptr);
                *ptr++ = sxth(cmp_value, reg, 0);
                break;
        }
    }

    *ptr++ = cmp_reg(cmp_value, lower);
    branch_skip_upper = ptr;
    *ptr++ = b_cc(ARM_CC_LE, 0);
    *ptr++ = cmp_reg(higher, cmp_value);
    *branch_skip_upper = b_cc(ARM_CC_LE, ptr - branch_skip_upper - 2);
    ...
    RA_FreeARMRegister(&ptr, ea);
    if (cmp_value != reg)
        RA_FreeARMRegister(&ptr, cmp_value);
    RA_FreeARMRegister(&ptr, lower);
    RA_FreeARMRegister(&ptr, higher);
    ...
    if (opcode2 & (1 << 11))
    {
        ...
        *branch_in_range = b_cc(ARM_CC_CS, ptr - branch_in_range - 2);
...
#define CAS_UNSAFE() do { \
    ...
    if ((opcode & 0xfdff) == 0x0cfc)
    {
        ...
        uint32_t *branch_first_mismatch;
        uint32_t *branch_second_mismatch;
        uint32_t *branch_done;
        uint32_t *mismatch_ptr;

        if (size == 2)
        {
            *ptr++ = ldrh_offset(rn1, val1, 0);
            *ptr++ = ldrh_offset(rn2, val2, 0);
            *ptr++ = lsl_immed(val1, val1, 16);
            *ptr++ = lsl_immed(val2, val2, 16);

            *ptr++ = mov_reg(cmp_tmp, dc1);
            *ptr++ = lsl_immed(cmp_tmp, cmp_tmp, 16);
            *ptr++ = cmp_reg(val1, cmp_tmp);
            branch_first_mismatch = ptr;
            *ptr++ = b_cc(ARM_CC_NE, 0);

            *ptr++ = mov_reg(cmp_tmp, dc2);
            *ptr++ = lsl_immed(cmp_tmp, cmp_tmp, 16);
            *ptr++ = cmp_reg(val2, cmp_tmp);
            branch_second_mismatch = ptr;
            *ptr++ = b_cc(ARM_CC_NE, 0);
            ...
            *ptr++ = b_cc(ARM_CC_AL, 0);
            ...
            mismatch_ptr = ptr;
            *ptr++ = lsr_immed(val1, val1, 16);
            *ptr++ = bfi(dc1, val1, 0, 16);
            *ptr++ = lsr_immed(val2, val2, 16);
            *ptr++ = bfi(dc2, val2, 0, 16);
        }
        else
        {
            *ptr++ = ldr_offset(rn1, val1, 0);
            *ptr++ = ldr_offset(rn2, val2, 0);
            *ptr++ = cmp_reg(val1, dc1);
            branch_first_mismatch = ptr;
            *ptr++ = b_cc(ARM_CC_NE, 0);
            *ptr++ = cmp_reg(val2, dc2);
            branch_second_mismatch = ptr;
            *ptr++ = b_cc(ARM_CC_NE, 0);
        ...
        *branch_first_mismatch = b_cc(ARM_CC_NE, mismatch_ptr - branch_first_mismatch - 2);
        *branch_second_mismatch = b_cc(ARM_CC_NE, mismatch_ptr - branch_second_mismatch - 2);
        *branch_done = b_cc(ARM_CC_AL, ptr - branch_done - 2);
...
#define CAS_COMPARE_AND_LOAD_ATOMIC() do { \
            switch (size) \
            { \
                case 1: \
                    *ptr++ = ldrexb(ea, tmp); \
                    *ptr++ = lsl_immed(tmp, tmp, 24); \
                    *ptr++ = mov_reg(status, dc); \
                    *ptr++ = lsl_immed(status, status, 24); \
                    *ptr++ = cmp_reg(tmp, status); \
                    break; \
                case 2: \
                    *ptr++ = ldrexh(ea, tmp); \
                    *ptr++ = lsl_immed(tmp, tmp, 16); \
                    *ptr++ = mov_reg(status, dc); \
                    *ptr++ = lsl_immed(status, status, 16); \
                    *ptr++ = cmp_reg(tmp, status); \
                    break; \
                case 3: \
                    *ptr++ = ldrex(ea, tmp); \
...
#define CAS_RESTORE_COMPARE_FLAGS() do { \
            switch (size) \
            { \
                case 1: \
                    *ptr++ = mov_reg(status, dc); \
...
#define CAS_COMPARE_AND_LOAD_UNSAFE() do { \
            switch (size) \
            { \
                case 1: \
                    *ptr++ = ldrb_offset(ea, tmp, 0); \
...
#define CAS_ATOMIC() do { \
            uint32_t *loop_ptr = ptr; \
            uint32_t *branch_mismatch; \
            uint32_t *branch_retry; \
            uint32_t *branch_done; \
            uint32_t *mismatch_ptr; \
            CAS_COMPARE_AND_LOAD_ATOMIC(); \
            branch_mismatch = ptr; \
            *ptr++ = b_cc(ARM_CC_NE, 0); \
            switch (size) \
            { \
                case 1: \
                    *ptr++ = strexb(ea, du, status); \
                    break; \
                case 2: \
                    *ptr++ = strexh(ea, du, status); \
                    break; \
                case 3: \
                    *ptr++ = strex(ea, du, status); \
                    break; \
            } \
            *ptr++ = cmp_immed(status, 0); \
            branch_retry = ptr; \
            *ptr++ = b_cc(ARM_CC_NE, 0); \
            CAS_RESTORE_COMPARE_FLAGS(); \
            branch_done = ptr; \
            *ptr++ = b_cc(ARM_CC_AL, 0); \
            mismatch_ptr = ptr; \
            CAS_WRITEBACK(); \
            *branch_mismatch = b_cc(ARM_CC_NE, mismatch_ptr - branch_mismatch - 2); \
            *branch_retry = b_cc(ARM_CC_NE, loop_ptr - branch_retry - 2); \
            *branch_done = b_cc(ARM_CC_AL, ptr - branch_done - 2); \
        } while (0)
...
#define CAS_UNSAFE() do { \
        ...
        else
        {
            ...
            *ptr++ = b_cc(ARM_CC_EQ, 0);
            ...
            branch_done = ptr;
            *ptr++ = b_cc(ARM_CC_AL, 0);
            *branch_atomic = b_cc(ARM_CC_EQ, ptr - branch_atomic - 2);
            CAS_ATOMIC();
            *branch_done = b_cc(ARM_CC_AL, ptr - branch_done - 2);
        }
...
#undef CAS_UNSAFE
#undef CAS_ATOMIC
#undef CAS_COMPARE_AND_LOAD_UNSAFE
#undef CAS_RESTORE_COMPARE_FLAGS
#undef CAS_COMPARE_AND_LOAD_ATOMIC
#undef CAS_WRITEBACK
...
uint32_t *EMIT_MOVEP(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr)
{
    ...
    if (offset != 0) {
        ...
        *ptr++ = movw_immed_u16(off, (uint16_t)(offset > 0 ? offset : -offset));
        if (offset > 0)
            *ptr++ = add_reg(addr, an, off, 0);
        else
            *ptr++ = sub_reg(addr, an, off, 0);
        offset = 0;
    ...
    if (opcode & 0x80) {
        /* Long mode */
        if (opcode & 0x40) {
            *ptr++ = lsr_immed(tmp, dn, 24);
            *ptr++ = strb_offset(addr, tmp, offset);
            *ptr++ = lsr_immed(tmp, dn, 16);
            *ptr++ = strb_offset(addr, tmp, offset + 2);
            *ptr++ = lsr_immed(tmp, dn, 8);
    ...
    /* Memory to register transfer */
        ...
        if (opcode & 0x40) {
            ...
            *ptr++ = lsl_immed(dn, dn, 24);
            *ptr++ = orr_reg(dn, dn, tmp, 16);
            *ptr++ = ldrb_offset(addr, tmp, offset + 4);
            *ptr++ = orr_reg(dn, dn, tmp, 8);
            *ptr++ = ldrb_offset(addr, tmp, offset + 6);
            *ptr++ = orr_reg(dn, dn, tmp, 0);
        }
        /* Word mode */
        else {
#ifdef __aarch64__
...
uint32_t *EMIT_MOVES(uint32_t *ptr, uint16_t opcode, uint16_t **m68k_ptr)
{
    ...
    /* Test if supervisor mode is active */
    *ptr++ = ubfx(tmpreg, cc, SRB_S, 1);
    *ptr++ = tst_immed(tmpreg, 1);
    ...
    *ptr++ = b_cc(ARM_CC_EQ, 0);
    ...
    // Transfer from EA to Register
        ...
        if (size == 4)
            ...
            if (opcode2 & 0x8000) {
                uint8_t tmpreg2 = 0xff;

                ptr = EMIT_LoadFromEffectiveAddress(ptr, size, &tmpreg2, opcode & 0x3f, *m68k_ptr, &ext_count, 0, NULL);

                switch (size)
                {
                    case 1:
                        *ptr++ = sxtb(reg, tmpreg2, 0);
                        break;

                    case 2:
                        *ptr++ = sxth(reg, tmpreg2, 0);
                        break;
                }

                RA_FreeARMRegister(&ptr, tmpreg2);
            }
            else {
                uint8_t tmpreg2 = 0xff;
                ...
                {
                    case 1:
                        *ptr++ = bfi(reg, tmpreg2, 0, 8);
                        break;
                
                    case 2:
                        *ptr++ = bfi(reg, tmpreg2, 0, 16);
                        break;
                }
